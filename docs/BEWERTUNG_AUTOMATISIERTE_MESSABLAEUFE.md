...
> 25.08.2026**. Prüfstand danach: 725 Tests, Abdeckung 90 %.
>
...
| Vollständiger Testlauf | **Nachgeholt am 25.08.2026:** zuletzt `725 passed, 1 skipped` in 1,07 s. Die Erstfassung konnte nicht messen, weil `pytest` in der dortigen Umgebung fehlte; die Suite läuft in einer eigenen Umgebung (`python3 -m venv`, `pip install -e ".[test]"`) ohne Gerät und ohne DLL. |
| Statische Prüfung | **Nachgeholt:** `ruff check .` und `mypy` ohne Befund. |
| Zeilenabdeckung | **Gemessen:** 90 % über das Paket (vor Zug 1: 87 %). Die frühere Schwachstelle `wt3000_ranging.py` steht nach **A6** bei 93 %. Schwächste Stelle ist jetzt `wt3000_input.py` mit 74 % — dort liegen die Setter, die am Gerät noch nicht abgenommen sind (**A1**). |
